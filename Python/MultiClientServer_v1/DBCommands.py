...
def commandDecorator(execFunc):
    def decorator(*args, **kwargs):
        try:
            ...
            return f'>>> {colored("ERROR","red")} ::::: {colored(e,"red")}'
    return decorator
...
class setCommand(commandAbstract):
    ...
    def description(self):
       return f'{self.command} ::: set one or multiple pair key-value | example: set a 10 ...'
...
class createDBCommand(commandAbstract):
    ...
    def description(self):
        return f'{self.command} ::: create database | example: cdb db1'
...
class useDBCommand(commandAbstract):
    ...
    def description(self):
        return f'{self.command} ::: select database to work with | example: udb db1'
...
class showDBCommand(commandAbstract):
    ...
    def exec(self, *args, **kwargs):
        ...
        for (k, v) in zip(db.keys(), db.values()):
            pairs+='{0} ::: {1}'.format(k, v)+'\n'
        return pairs

    def description(self):
        return f'{self.command} ::: print all database key-value pairs | example: pdb db1'
...
class printDBCommand(commandAbstract):
    ...
    def exec(self, *args, **kwargs):
        ...
        for db in list(DBs.keys()):
            dbs+=f'{db}\n'
        return ' ' if dbs=='' else dbs

    def description(self):
        return f'{self.command} ::: show databases | example: shdb'
...
class printDBsFilesCommand(commandAbstract):
    ...
    def exec(self, *args, **kwargs):
        files=''
        for f in getDBsFiles():
            files+=f'{f}\n'
        return ' ' if files =='' else files

    def description(self):
        return f'{self.command} ::: print all saved csv files | example: pdbfs'
...
class currdbCommand(commandAbstract):
    ...
    @commandDecorator
    def exec(self, *args, **kwargs):
        return currDB['name_db']


    def description(self):
        return f'{self.command} ::: print current database name | example: crdb'
...
class showCmdListCommand(commandAbstract):
    ...
    def description(self):
        return f'{self.command} ::: show list of available commands | example: shcmdl'
...
class concatDBsCommand(commandAbstract):
    ...
    def description(self):
        return f'{self.command} ::: concatenates 2 or more databases; data added to first defined database | example: cnct db1 db2 db3 ...'
...
class saveDBCommand(commandAbstract):
    ...
    def description(self):
        return f'{self.command} ::: save one or several databases to csv format | example: fave db1 db2 ...'
...
class removeDBCommand(commandAbstract):
    ...
    def description(self):
        return f'{self.command} ::: remove one or several database | example: rmdb db1 db2 ...'
...
class removeDBFileCommand(commandAbstract):
    ...
    def description(self):
        return f'{self.command} ::: remove one or several databases csv file | example: rmdbf db1 db2 ...'
...
class loadDBCommand(commandAbstract):
    ...
    def description(self):
        return f'{self.command} ::: load one or several databases from csv | example: loaddb db1 db2 ...'
...
class exitCommand(commandAbstract):
    ...
    def description(self):
        return f'{self.command} ::: exit from database management system | example: exit'
